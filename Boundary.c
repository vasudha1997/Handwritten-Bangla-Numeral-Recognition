...
void sample_processing( char* s )
{
	...
	for( int i=0; i<row; i++ )
	{
		for( int j=0; j<col; j++ )
		{
			if( *(image+i*col+j) == 0 &&( *(image+(i-1)*col+j-1)==255||*(image+(i-1)*col+j)==255||*(image+(i-1)*col+j+1)==255||*(image+i*col+j-1)==255||*(image+i*col+j+1)==255||*(image+(i+1)*col+j-1)==255||*(image+(i+1)*col+j)==255||*(image+(i+1)*col+j+1)==255 ))
				*(boundary+i*col+j) = 0;
		}
	}	
}
